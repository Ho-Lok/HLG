...
MODEL_PATH = os.path.join(MODEL_SAVE_DIR, "xgboost_live_20.json")
...
def extract_all_features(files, scaler, schema_cols):
    ...
    for f in tqdm(files, desc="Extracting XGBoost NO-DS Data"):
        ...
        try:
            ...
            if len(df_price) < (LOOKBACK_WINDOW + PREDICTION_HORIZON + 5):
                continue
            ...
            if close_col is None or "SPY_Close" not in df_price.columns:
                continue
            ...
            if os.path.exists(vec_path):
                ...
                df = pd.concat(
                    [df_price, pd.DataFrame(0.0, index=df_price.index, columns=[f"dim_{i+1}" for i in range(VECTOR_DIM)])],
                    axis=1,
                )
            ...
            df_scaled_values = (
                df_feat.reindex(columns=schema_cols, fill_value=0.0).replace([np.inf, -np.inf], np.nan).fillna(0.0)
            )
