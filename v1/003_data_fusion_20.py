...
class FusionEngine:
    ...
    def load_ticker_data(self, ticker, start_date="2016-01-01", end_date="2024-01-01"):
        ...
        try:
            ...
            if len(df_price) < 100: return None

            full_cal = pd.date_range(start=df_price.index.min(), end=df_price.index.max(), freq='D')

            # FinBERT Vectors (Shifted & Smoothed)
            train_vec_path = os.path.join(VECTOR_DIR_TRAIN, f"{ticker}_vec.csv")
            if os.path.exists(train_vec_path): 
                df_vec = pd.read_csv(train_vec_path, parse_dates=['date']).set_index('date').sort_index()
    ...
    def fit_scalers(self, files):
        ...
        for f in tqdm(sampled_files): 
            ...
            if df is not None:
                ...
                if len(df_train_only) < 50: continue
                ...
                df_feat[cols_to_pct] = df_feat[cols_to_pct].pct_change().fillna(0.0).replace([np.inf, -np.inf], 0.0).clip(-5.0, 5.0)
                df_feat[cols_to_diff] = df_feat[cols_to_diff].diff().fillna(0.0).replace([np.inf, -np.inf], 0.0).clip(-5.0, 5.0)
    ...
    def create_sequences(self, df, ticker):
        ...
        if df_feat is None: return None, None
        
        try: data_scaled = self.scaler.transform(df_feat.values)
...
if __name__ == "__main__":
    ...
    for i, f in enumerate(tqdm(files)):
        X, y = engine.create_sequences(engine.load_ticker_data(f.replace("_daily.csv", ""), start_date="2016-01-01", end_date="2024-01-01"), f)
        ...
        if (i + 1) % BATCH_SIZE == 0 or (i + 1) == len(files):
            ...
            np.savez_compressed(os.path.join(OUTPUT_DIR, f"train_batch_nodis_{batch_id}.npz"), 
                                X=np.concatenate(batch_X, axis=0), y=np.concatenate(batch_y, axis=0))
            batch_id += 1
            batch_X, batch_y = [], []
            gc.collect()
